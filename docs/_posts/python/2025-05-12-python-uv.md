...
pip와 poetry는 [resolvelib](https://github.com/sarugaku/resolvelib){:target="_blank"} 알고리즘을 사용하였지만, uv는 [PubGrub](https://github.com/pubgrub-rs/pubgrub){:target="_blank"} 알고리즘을 사용하여 의존성 충돌을 훨씬 빠르면서도 정확하게 해결할 수 있습니다.
이러한 속도는 프로젝트의 복잡도가 커질 수록 극명하게 차이가 납니다.
...
또한 GitHub Workflow에서 테스트 코드 실행 시에는 아래와 같이 실행할 수 있습니다.
...
위 스크립트는 pytest 기준이며, Django 테스트 실행시에는 `uv run manage.py test` 와 같이 작성할 수 있습니다.
